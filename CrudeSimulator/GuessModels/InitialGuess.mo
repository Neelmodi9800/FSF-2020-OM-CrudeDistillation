...
//Generation of Bubble Temperature for DC guess-(Temperatures)
      ...
      for i in 1:Nc loop
        Psatbg[i] = CrudeSimulator.Files.ThermodynamicFunctions.Psat(C[i].VP, Tbg);
        Psatdg[i] = CrudeSimulator.Files.ThermodynamicFunctions.Psat(C[i].VP, Tdg);
      end for;
      Fg = Fliqg + Fvapg;
...
//============================================================================
      Tc = C.Tc;
      for i in 1:Nc loop
        Psatt[i] = CrudeSimulator.Files.ThermodynamicFunctions.Psat(C[i].VP, Temp);
...
//Generating the temperature in case of PH Flash
      ...
      for i in 1:Nc loop
        ...
        Hcomplg[i] = CrudeSimulator.Files.ThermodynamicFunctions.HLiqId(C[i].SH, C[i].VapCp, C[i].HOV, C[i].Tc, Temp);
        Hcompvg[i] = CrudeSimulator.Files.ThermodynamicFunctions.HVapId(C[i].SH, C[i].VapCp, C[i].HOV, C[i].Tc, Temp);
      end for;
      Htotg = Hliqg + Hvapg;
